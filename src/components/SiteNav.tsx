  ...
  return (
    <>
      <button
        ...
        className="fixed right-[clamp(1rem,4vw,2rem)] top-[clamp(1rem,4vw,1.75rem)] z-[60] flex items-center gap-3 rounded-[1584px] bg-char px-4 py-2.5 text-paper"
      >
        ...
        <span className="relative block h-[9px] w-4">
          <span
            className={`absolute left-0 block h-px w-full bg-paper transition-all duration-300 ${
              open ? "top-1 rotate-45" : "top-0"
            }`}
          />
          <span
            className={`absolute left-0 top-1 block h-px w-full bg-paper transition-all duration-300 ${
              open ? "-rotate-45" : ""
            }`}
          />
          <span
            className={`absolute left-0 block h-px w-full bg-paper transition-all duration-200 ${
      ...
      <div
        className={`fixed inset-0 z-50 bg-midnight text-paper transition-opacity duration-300 ${
          ...
          <nav className="shell grid flex-1 content-center gap-4 py-16 sm:grid-cols-2">
            {nav.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="group flex items-stretch gap-4 rounded-[8px] bg-char p-3 transition-colors hover:bg-iron"
              >
                <div className="w-2/5 shrink-0 overflow-hidden rounded-[6px]">
                  <ImagePlaceholder
                    variant="bleed"
                    ratio="4 / 3"
                    className="h-full bg-slate/50 text-paper/30"
                    label="Imagem"
                  />
                </div>
                <div className="flex flex-1 flex-col justify-between py-1 pr-2">
                  <div>
                    ...
                    <p className="mt-1 text-[12px] leading-snug text-mist/80">
                      {item.desc}
                    </p>
                  </div>
                  <Arrow className="h-3.5 w-3.5 self-end text-paper transition-transform duration-300 group-hover:translate-x-1" />
                </div>
              </Link>
            ))}
          </nav>
